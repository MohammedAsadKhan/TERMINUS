...
function IncidentDetail({ id, close }: { id: string; close: () => void }) {
  ...
  return <Drawer title={<span className="mono">{id}</span>} size={700} open onClose={close} extra={t && <Button icon={<DownloadOutlined />} onClick={() => download(`${t.id}.json`, JSON.stringify(t, null, 2))}>Export</Button>}>
    {incident.isPending ? <Loading /> : incident.error ? <ErrorPanel error={incident.error} retry={() => void incident.refetch()} /> : t && <div className="detail-stack">
      ...
      <section className="detail-section"><div className="eyebrow">RECOMMENDED NEXT STEPS</div><ol className="recommendations">{t.recommended_actions.map((item, i) => <li key={i}>{item}</li>)}</ol><Alert type="info" showIcon title="Analyst Action Required" description="Verify threat indicators and host context before approving automated remediation directives." /></section>
...
export default function Operations({ incidentView = false }: { incidentView?: boolean }) {
  ...
  return <><PageTitle eyebrow={incidentView ? 'INVESTIGATE' : 'OPERATIONS / OVERVIEW'} title={incidentView ? 'Incident queue' : 'Your security, in focus.'} description={incidentView ? 'Review the evidence. Prioritize what needs your attention.' : 'A clear view of incoming threats and the work ahead.'} actions={<Space><Button type="primary" icon={<ReloadOutlined spin={incidents.isFetching} />} onClick={() => void incidents.refetch()}>Refresh</Button></Space>} />
    ...
    <div className="overview-grid"><section className="panel signal-panel"><div className="panel-heading"><div><span className="eyebrow">THREAT DISTRIBUTION</span><h2>Severity Breakdown</h2></div><Tag bordered={false}>CURRENT SESSION</Tag></div><div className="distribution-bar">{data.length ? counts.filter(c => c.count).map(c => <div key={c.name} className={`segment ${c.name}`} style={{ flex: c.count }} title={`${c.name}: ${c.count}`} />) : <div className="segment no-data" />}</div><div className="distribution-legend">{counts.map(c => <div key={c.name}><i className={`legend-dot ${c.name}`} /><span>{c.name}</span><strong>{c.count}</strong></div>)}</div><div className="panel-bottom">{data.length ? `${data.length} incident assessments · ${open.length} open` : 'Awaiting incoming telemetry streams from monitored endpoints.'}</div></section>
    <section className="panel pipeline-panel"><div className="panel-heading"><div><span className="eyebrow">IMPACTED ASSETS</span><h2>Monitored Endpoints</h2></div><Tag bordered={false}>{impactedHosts.length} ACTIVE</Tag></div>
    {impactedHosts.length ? <div className="endpoint-list">{impactedHosts.slice(0, 3).map(item => <div key={item.host} className="pipeline-step"><span style={{ fontSize: 13 }}><DesktopOutlined /></span><div><Link to={`/incidents?q=${encodeURIComponent(item.host)}`}><strong>{item.host}</strong></Link><small>{item.count} {item.count === 1 ? 'incident' : 'incidents'} · Last active {date(item.latestTime)}</small></div><Severity value={item.maxSeverity} /></div>)}</div> : <div style={{ padding: '24px 0', textAlign: 'center' }}><CheckCircleOutlined style={{ fontSize: 24, color: '#a4dfba', marginBottom: 8, display: 'block' }} /><strong style={{ display: 'block', fontSize: 13 }}>All Endpoints Nominal</strong><small className="muted">No compromised hosts detected across monitored fleet.</small></div>}
    <div className="panel-bottom">{impactedHosts.length ? `${impactedHosts.length} monitored host${impactedHosts.length > 1 ? 's' : ''} reporting telemetry anomalies` : 'Telemetry sensor fleet reporting zero active threats.'}</div>
    </section></div></>}
    <section className="panel table-panel"><div className="panel-heading"><div><span className="eyebrow">{incidentView ? 'ALL INCIDENTS' : 'RECENT ACTIVITY'}</span><h2>{incidentView ? 'Investigation workspace' : 'Latest incidents'}</h2></div>{!incidentView && <Link to="/incidents">View all <ArrowRightOutlined /></Link>}</div>
      ...
      <Table<Incident> rowKey="id" columns={columns} dataSource={incidentView ? filtered : data.slice(0, 5)} loading={incidents.isPending} scroll={{ x: 900 }} pagination={incidentView ? { pageSize: 10, showSizeChanger: false, showTotal: total => `${total} incidents` } : false} locale={{ emptyText: <EmptyPanel title={data.length ? 'No matching incidents' : 'No active incidents'} description={data.length ? 'Try adjusting your filters.' : 'All telemetry streams are clear. Incoming security events will appear here automatically.'} /> }} />
