...
export default function Settings({ section }: { section: 'integrations' | 'organization' | 'settings' }) {
  ...
  if (section === 'integrations') return <><PageTitle eyebrow="CONFIGURE / SOURCES & INTEGRATIONS" title="Telemetry & Integrations" description="Inspect security event sources, SIEM connectors, and notification endpoints configured for your SOC." actions={<Button type="primary" icon={<ExperimentOutlined />} disabled={!detail || detail.role === 'viewer'} onClick={() => setIngest(true)}>Simulate Event</Button>} />
    <section className="panel source-banner"><span className="source-icon"><ApiOutlined /></span><div><div className="eyebrow">AUTHENTICATED INGESTION</div><h2>Automated Ingestion Pipeline</h2><p>Submit Wazuh-compatible JSON alerts for policy evaluation and investigation.</p></div><Tag color="green">POST /wazuh</Tag></section>
    ...
    <div className="settings-grid"><section className="panel"><div className="eyebrow">INGESTION PIPELINE API</div><h2>Direct Alert Dispatch</h2><p className="muted">Authorize inbound alerts via Bearer session tokens with tenant organization headers. Payloads are validated against standard SOC telemetry schemas.</p><Code>{`curl -X POST '${window.location.origin}/wazuh' \\\n  -H 'Authorization: Bearer <SESSION_TOKEN>' \\\n  -H 'X-Org-ID: ${orgId}' \\\n  -H 'Content-Type: application/json' \\\n  -d '{"id":"source-event-001","rule_id":5710,"level":8,"description":"SSH failures","full_log":"sshd: failed password"}'`}</Code></section><section className="panel"><div className="eyebrow">INTEGRATION ARCHITECTURE</div><h2>Security Connectors</h2><p className="muted">Connector credentials are configured on the server and currently shared across organizations.</p><Alert type="info" title="Connector configuration" description="Configured means settings are present. Connection health and per-organization external credentials are not verified here." showIcon /></section></div>
    <Modal open={!!connection} title={connection?.name + ' configuration'} onCancel={() => setConnection(null)} footer={<Button onClick={() => setConnection(null)}>Close</Button>}><p>{connection?.description}</p><p className="muted">Set the corresponding TERMINUS environment variables on the server and restart. Secret values are never returned to the console.</p><Code>{`${connection?.setting}=<configured on the server>`}</Code><Alert type={connection?.configured ? 'success' : 'info'} title={connection?.configured ? 'Settings present; connection unverified' : 'Connector awaiting credentials'} /></Modal><IngestModal open={ingest} close={() => setIngest(false)} />
  </>;
  ...
  return <><PageTitle eyebrow="GOVERN / SETTINGS" title="Platform Configuration" description="Review runtime environment, cryptographic license entitlements, and active SOC triage policies." />
    <div className="stats-grid"><Stat label="STORAGE ENGINE" value="In-Memory" note="Records are lost when the server restarts" /><Stat label="AI ENGINE" value={system?.llm_mode === 'remote' ? 'Remote model' : 'Scripted demo'} note={system?.llm_model || 'TERMINUS AI Engine'} /><Stat label="CONSOLE UPDATES" value="Polling" note="Incident refresh every 5 seconds" /><Stat label="LICENSE TIER" value={detail?.license?.tier.toUpperCase() || '—'} note={detail?.license ? `Expires ${date(detail.license.expires_at)}` : 'No valid license'} /></div>
      ...
      <div className="panel-heading" style={{ marginBottom: 4 }}>
        <div>
          ...
          <p className="muted" style={{ margin: '4px 0 0', fontSize: 12 }}>
            Switch between specialized operations center color palettes. Themes apply dynamically across telemetry charts, navigation panels, and action buttons, saved in this browser.
          </p>
        </div>
        ...
        {availableThemes.map(t => {
          ...
          return (
            <div
              ...
              onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setThemeId(t.id); } }}
    ...
    <div className="settings-grid"><section className="panel"><div className="eyebrow">LICENSE MANAGEMENT</div><h2>Enterprise Entitlement</h2>{detail?.license_error && <Alert type="error" title={detail.license_error} />}<p className="muted">HMAC-signed license tokens define organization entitlements and seat limits. Automated response execution is not implemented.</p><Space wrap>{detail?.license?.features.map(feature => <Tag key={feature} icon={<CheckCircleOutlined />}>{feature.replaceAll('_', ' ')}</Tag>)}</Space><Form layout="vertical" onFinish={values => license.mutate(values)}><Form.Item name="token" label="Signed license token" rules={[{ required: true }]}><Input.TextArea rows={3} disabled={!canWrite} placeholder="Paste the license token for this organization" /></Form.Item><Button type="primary" htmlType="submit" icon={<KeyOutlined />} disabled={!canWrite} loading={license.isPending}>Activate license</Button></Form></section>
      <section className="panel"><div className="eyebrow">TRIAGE POLICY</div><h2>Signal Prioritization Rules</h2><div className="policy-row"><Tag color="blue">IGNORE</Tag><span>Severity below 5, without a MITRE ATT&amp;CK override</span></div><div className="policy-row"><Tag color="gold">TRIAGE</Tag><span>Severity 5–9, without a MITRE ATT&amp;CK override</span></div><div className="policy-row"><Tag color="red">ESCALATE</Tag><span>Severity 10+ or a recognized MITRE technique format (Txxxx)</span></div><p className="muted">Policy rules evaluate incoming alerts before sending eligible alerts to the configured investigation model.</p></section></div>
    <section className="panel"><div className="eyebrow">ACTIVE ACCOUNT</div><h2>{user.display_name}</h2><Descriptions column={{ xs: 1, md: 3 }} items={[{ key: 'email', label: 'Email', children: user.email }, { key: 'id', label: 'User ID', children: <Button type="text" icon={<CopyOutlined />} onClick={() => { void navigator.clipboard.writeText(user.user_id); message.success('User ID copied'); }}>{user.user_id}</Button> }, { key: 'session', label: 'Session', children: 'HttpOnly session cookie · 12-hour expiry' }]} /></section>
  </>;
}
