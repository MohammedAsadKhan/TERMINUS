...
export function ConsoleApp() {
  ...
  return <SessionContext.Provider value={{ user: user.data, orgId, detail: detail.data, system: system.data }}><div className="workspace">
    ...
    </aside><main className="workspace-main"><header className="topbar"><Space><Button className="mobile-menu" type="text" aria-label="Open navigation" icon={<MenuOutlined />} onClick={() => setMobileOpen(true)} /><span className="breadcrumb-org">{organization?.name || 'Welcome'}</span><span className="breadcrumb-slash">/</span><strong>{currentPage}</strong></Space><Space size={16}><Tooltip title={system.isError ? 'Cannot reach the SOC API service' : 'Telemetry and incident sync operational.'}><span className="connection"><i className={`status-dot ${system.isError ? 'error' : ''}`} />{system.isError ? 'Service Offline' : 'Operational'}</span></Tooltip></Space></header>
