...
import { ApartmentOutlined, ArrowRightOutlined, BgColorsOutlined, DeploymentUnitOutlined, FileTextOutlined, GoldOutlined, LogoutOutlined, MenuOutlined, PlusOutlined, SafetyCertificateOutlined, SettingOutlined, ThunderboltOutlined, UserOutlined } from '@ant-design/icons';
...
export function ConsoleApp() {
  ...
  return <SessionContext.Provider value={{ user: user.data, orgId, detail: detail.data, system: system.data }}><div className="workspace">
    ...
    <aside className={`sidebar ${mobileOpen ? 'open' : ''}`}><Link to="/" aria-label="Terminus overview"><Brand /></Link><div className="workspace-label">WORKSPACE</div><div className="org-switch"><Select aria-label="Active organization" value={orgId || undefined} placeholder="Select organization" options={orgs.data?.map(org => ({ value: org.org_id, label: org.name }))} onChange={switchOrg} /><Tooltip title="Create organization"><Button type="text" aria-label="Create organization" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)} /></Tooltip></div>
      ...
      <div className="sidebar-bottom"><div className="environment-note"><span className="status-dot" />SOC Engine Active<small>Autonomous Triage &amp; Defense</small></div><Dropdown menu={{ items: [{ key: 'account', label: 'Copy my user ID', onClick: () => { void navigator.clipboard.writeText(user.data.user_id); message.success('User ID copied'); } }, { key: 'logout', label: 'Sign out', icon: <LogoutOutlined />, onClick: () => void logout() }] }} trigger={['click']}><button className="user-menu"><Avatar shape="square" size={32}>{user.data.display_name[0]?.toUpperCase()}</Avatar><span>{user.data.display_name}<small>{detail.data?.role || 'Account'}</small></span><span className="user-chevron">⌄</span></button></Dropdown></div>
    </aside><main className="workspace-main"><header className="topbar"><Space><Button className="mobile-menu" type="text" aria-label="Open navigation" icon={<MenuOutlined />} onClick={() => setMobileOpen(true)} /><span className="breadcrumb-org">{organization?.name || 'Welcome'}</span><span className="breadcrumb-slash">/</span><strong>{currentPage}</strong></Space><Space size={16}><Dropdown menu={{ items: availableThemes.map(t => ({ key: t.id, label: <Space><span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', background: t.primaryColor }} /><span>{t.name}</span>{t.id === themeId && <Tag color="success" style={{ fontSize: 9, margin: 0 }}>Active</Tag>}</Space>, onClick: () => { setThemeId(t.id); message.success(`Theme set to ${t.name}`); } })) }} trigger={['click']}><Button type="text" size="small" icon={<BgColorsOutlined style={{ color: activeTheme.primaryColor }} />}><span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{activeTheme.name}</span></Button></Dropdown><Tooltip title={system.isError ? 'Cannot reach the SOC API service' : 'Telemetry and incident sync operational.'}><span className="connection"><i className={`status-dot ${system.isError ? 'error' : ''}`} />{system.isError ? 'Service Offline' : 'Operational'}</span></Tooltip></Space></header>
      ...
      </div><footer className="workspace-footer"><span>TERMINUS / ANALYST CONSOLE</span><span>TERMINUS Enterprise SOC Platform v0.2.0</span></footer>
